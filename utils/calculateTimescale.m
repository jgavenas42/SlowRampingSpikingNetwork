...
% 2) tau needs to be between 0 and 500 ms.
...
% in case we want to use this function on firing rates that are already
% specified
FR = nan;
...
if isnan(FR)
    if ndims(spk_mat) > 2
        spk_mat = squeeze(spk_mat(~isnan(spk_mat)));
    end
    FR = firingRate(spk_mat(~isnan(spk_mat(:,1)),:),'count',true,'bin_size',50);
end
inclusion = zeros(5,1);

if nanmean(FR,'all') < 2.5
    inclusion(1) = 1;
    tau_intrinsic = nan;
end
...
FR_corr_algn = alignMat(FR_corr,false);

acf = nanmean(FR_corr_algn);

acf_diff = diff(acf(2:end));
